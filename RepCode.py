...
def _writeStruct(theVal, theStream, theStruct):
    ...
    if l != theStruct.size:
        raise ExceptionRepCodeWriteToStream(
            'Only able to write {:d} bytes but expected to write {:d} bytes'.format(l, theStruct.size))
    return l
...
def readUVARI(theS):
    ...
    b = theS.read(1)[0]
    bn = "{0:08b}".format(b)
    #print("readUVARI: {0:08b}".format(b))
    if bn[0] == "1":
        if bn[1] == "1":
            b &= 0x3F
            for i in range(3):
...
class PascalLikeBase(object):
    ...
    def _nullValue(self):
        if isinstance(self._payload, str):
            ...
            raise ExceptionRepCodePascalLikeBase(
                '_nullValue(): unrecognised internal type {:s}'.format(type(self._payload)))
    ...
    def _checkValidChars(self):
        ...
        for i, c in enumerate(self._payload):
            ...
            if c not in self.ASCII_CHARS:
                #print(c, type(c))
                if c not in ["ã","â","Ã","Â","õ","ô","Õ","Ô","á","é","í","ó","ú","Á","É","Í","Ó","Ú"]:
